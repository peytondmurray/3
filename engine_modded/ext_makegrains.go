...
// Periodic boundary conditions for grains, defined in ext_makegrains3d
/*var (
	vPBCx = 0
	vPBCy = 0
	vPBCz = 0
...
func (t *tesselation) RegionOf(x, y, z float64) int {
	...
	mindist := math.Inf(1)
	ws := Mesh().WorldSize() //.WorldSize()
	xU := t.tileOf(ws[0]/2,0).x
	xL := t.tileOf(-ws[0]/2,0).x
	yU := t.tileOf(0,ws[1]/2).y
	yL := t.tileOf(0,-ws[1]/2).y
	for tx := tile.x - 1; tx <= tile.x+1; tx++ {
		...
		for ty := tile.y - 1; ty <= tile.y+1; ty++ {
			...
			centers := t.centersInTile(txPBC, tyPBC)
